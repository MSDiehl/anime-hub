...
export default function AnimeDetails({ onLogout }: Props) {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                const json = await apiGet<AnimeDetailsDto>(
                    `/api/anime/${activeId}`,
                );
                ...
                if (!cancelled) {
                    setDetailsError(
                        getErrorMessage(e, "Failed to load anime details"),
                    );
                    setData(null);
                }
            } finally {
    ...
    useEffect(() => {
        ...
        async function loadTrackedShow() {
            ...
            try {
                const json = await apiGet<TrackedShow>(
                    `/api/tracked/${activeId}`,
                );
    ...
    function optimisticTrackedShow(overrides: Partial<TrackedShow> = {}) {
        ...
        return {
            ...
            updatedUtc: null,
            ...overrides,
            trackingStatus: (overrides.trackingStatus ??
                payload.trackingStatus) as TrackingStatus,
        } as TrackedShow;
    }
    ...
    async function trackCurrent(overrides: Partial<TrackedShow> = {}) {
        ...
        try {
            setTrackedShow(
                await apiSend<TrackedShow>("/api/tracked", "POST", payload),
            );
            ...
            if (e instanceof ApiClientError && e.status === 409) {
                try {
                    setTrackedShow(
                        await apiGet<TrackedShow>(`/api/tracked/${activeId}`),
                    );
                } catch {
    ...
    async function loadThreads() {
        ...
        try {
            ...
            const json = await apiGet<ThreadPage | ThreadSummary[]>(
    ...
    const maxEpisodes =
        data.episodes && data.episodes > 0 ? data.episodes : undefined;
                ...
                <section className="adPanel adTrackingPanel mangaPanel">
                    ...
                    <div className="adTrackingTop">
                        ...
                        <div className="adTrackingActions">
                            ...
                            <button
                                className={[
                                    "adPillBtn",
                                    trackedShow?.isFavorite
                                        ? "adFavoriteBtn isActive"
                                        : "adFavoriteBtn",
                                ].join(" ")}
                                onClick={() =>
                                    patchTrackedShow({
                                        isFavorite: !(
                                            trackedShow?.isFavorite ?? false
                                        ),
                                    })
                                }
                                disabled={trackingSaving || trackingLoading}
                    ...
                    <div className="adTrackingGrid">
                        <label className="adTrackingField">
                            ...
                            <select
                                value={
                                    trackedShow?.trackingStatus ?? "PlanToWatch"
                                }
                                onChange={(e) =>
                                    patchTrackedShow({
                        ...
                        <label className="adTrackingField">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    patchTrackedShow({
                                        startedOn: dateOrNull(e.target.value),
                                    })
                                }
                                disabled={!isTracked || trackingSaving}
                            />
                        ...
                        <label className="adTrackingField">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    patchTrackedShow({
                                        completedOn: dateOrNull(e.target.value),
                                    })
                                }
                                disabled={!isTracked || trackingSaving}
                            />
                        ...
                        <div className="adPeopleList">
                            {(data.staff ?? []).length === 0 ? (
                                ...
                                data.staff?.map((member) => (
                                    <div
                                        key={`${member.aniListId}-${member.role}`}
                                        className="adPersonCard"
                                    >
                                        <div
                                            className="adPersonImage"
                    ...
                    {!episodes.length ? (
                        <EmptyState
                            ...
                            onAction={() => {
                                setDiscEpisode(null);
                                document
                                    .getElementById("discussion-panel")
                                    ?.scrollIntoView({
                                        behavior: "smooth",
                                        block: "start",
                                    });
                                ...
                                return (
                                    <div key={ep.num} className="adEpCard">
                                        ...
                                        <div className="adEpActions">
                                            <label
                                                className="adEpisodeCheck"
                                                title="Save episode progress"
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={watched}
                    ...
                    <div className="adDiscussionTop">
                        ...
                        <div className="adDiscussionControls">
                            ...
                            <button
                                className={
                                    "adScopeBtn" +
                                    (spoilerSafe ? " isActive" : "")
                                }
                                onClick={() =>
                                    setSpoilerSafe((value) => !value)
                                }
                            >
                                Spoiler-safe
                            </button>
                            ...
                            <div className="adThreadList">
                                ...
                                {threadsLoading ? (
                                    Array.from({ length: 3 }, (_, index) => (
                                        <div
                                            className="adThreadItem adThreadSkeleton"
                                            key={index}
                                        >
                                ...
                                <div className="adThreadView">
                                    ...
                                    <div className="adReplyBox">
                                        ...
                                        <div className="adReplyHint">
                                            Tip:{" "}
                                            <span className="adKbd">Ctrl</span>{" "}
                                            +{" "}
                                            <span className="adKbd">Enter</span>{" "}
                                            to send
                                        </div>
                                    </div>
                                </div>
