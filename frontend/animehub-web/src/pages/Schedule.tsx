...
export default function Schedule({ onLogout }: Props) {
    ...
    useEffect(() => {
        ...
        const timers = items
            .filter(
                (item) => item.isTracked && item.airingAt * 1000 > Date.now(),
            )
            .map((item) => {
    ...
    const filteredItems = useMemo(() => {
        ...
        return items
            ...
            .filter(
                (item) =>
                    !query || clampTitle(item).toLowerCase().includes(query),
            )
            ...
            <main className="schedMain">
                <div className="schedHeaderRow">
                    <div className="schedTitleBlock">
                        <h1 className="schedH1">
                            Release Schedule{" "}
                            <span className="schedBang">!</span>
                        </h1>
                        <div className="schedSub">
                            Times shown in {localTimeZone}. Cached schedule data
                ...
                <div className="schedFilterPanel">
                    ...
                    <button
                        ...
                        title={
                            notificationsEnabled
                                ? "Tracked episode notifications are enabled"
                                : "Notify when tracked episodes air"
                        }
                    >
                        {notificationsEnabled
                            ? "Notifications on"
                ...
                {loading ? (
                    ...
                    >
                        {visibleDays.map((day) => {
                            ...
                            const trackedItems = dayItems.filter(
                                (item) => item.isTracked,
                            );
                            const untrackedItems = dayItems.filter(
                                (item) => !item.isTracked,
                            );
            ...
            <div className="schedCardBody">
                ...
                <div className="schedMetaRow">
                    <span className="schedMetaChip">
                        {formatTimeLocalFromUnix(item.airingAt)}
                    </span>
                    {item.format && (
                        <span className="schedMetaChip">
                    ...
                    {item.isTracked && (
                        <span className="schedMetaChip isTracked">Tracked</span>
                    )}
                    {item.isWatched && (
                        <span className="schedMetaChip isWatched">Watched</span>
                    )}
                </div>
